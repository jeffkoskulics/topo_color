function plot_dir_fourier(parent_dir,type)
...
for z = 1:length(File_List)
    ...
    plot_fourier(double(elevation));
    
    title([save_name,' Fourier Plot'])
    
    if strcmp(type,'fig')
        saveas(gcf,[parent_dir,'Plots/',sub_dir,save_name,'_FourierPlot.fig'],'fig');
    end
    
    if strcmp(type,'tif')
        saveas(gcf,[parent_dir,'Plots/',sub_dir,save_name,'_FourierPlot.tif'],'tiff');
    end
