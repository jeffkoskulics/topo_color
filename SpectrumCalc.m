function [spectra_cube,X,Y,PL] = SpectrumCalc(directory,z_fp,fp_res,nadir,r_ap)
...
X_locations = [-.3023 -.216 -.1295 -.0432 .0432 .1295 .216 .3023];
Y_locations = [-.2886 -.1366 -.04445 .0476 .1397 .2318];
...
% unix style
[part,remainder] = strtok(directory,'/\');
if isunix
    directory = ['/',part];
else
    directory = part;
...
display('Setting X and Y locations for spectral readings...')
...
display('Fitting 5th order polynomial to spectral grid...')
m = 1;
for z = start_row:end_row %The usable wavelengths
    ...
    % Fit the values
    spectra_fitting(m) = polyfitn([X_vec Y_vec],spec,5);
    m = m + 1;

end

%% Define x,y locations to evaluate polyval at
display('Evaluating polynomial model on a 100x100 grid...')
...
fitted_spectrum = zeros(length(spectra_fitting),(x_numpoints*y_numpoints));
...
display('Calculating scattering constants...')
%Rayleigh scattering (Morel 1974)
scattering = (wavelengths/129).^(-4.32);

display('Creating data cube...')
...
for z = 1:length(PL);
    transmission = exp(-(absorption+scattering)*PL(z));
    for x = 1:x_numpoints
        for y = 1:y_numpoints
            spectra_cube(x,y,z,1) = sum(fitted_spectrum(:,x,y).*transmission.*red_sens);
            spectra_cube(x,y,z,2) = sum(fitted_spectrum(:,x,y).*transmission.*green_sens);
            spectra_cube(x,y,z,3) = sum(fitted_spectrum(:,x,y).*transmission.*blue_sens);
        end
    end
end

display('Create a 3-D matrix holding the 100x100x50 XxYxPL points...')
...
display('Creating flat surface points and surface normals...')
...
display('Place this in the parent directory, it contains the information necessary to run the analysis')
save([directory,'model_constants.mat'],'spectra_cube','X','Y','PL','flat_surf','u_fp','u_sn')
